		...
		public bool Paint(RaycastHit hit, List<P3dWindowPaintable> paintables)
		{
			switch (Technique)
			{
				case P3dWindowBrushTechnique.Replace:
				{
					...
					{
						paintables[i].Paint(Group, command, paintables);
					}
				}
				return true;
				...
				case P3dWindowBrushTechnique.Decal:
				{
					...
					P3dPainter.Decal.SetMatrix(hit.point, hit.normal, Angle, Radius, Texture, Depth, false);
					P3dPainter.Decal.SetMaterial(BlendMode, Texture, Hardness, NormalFront, NormalBack, NormalRange, Color, Opacity, Shape);
